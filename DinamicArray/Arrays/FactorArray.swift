...
final class FactorArray<T>: DynamicArray {
    ...
    private func resize() {
        var newArray = Array(repeating: defaultValue, count: (size() + 1) * factor) // new array
    ...
    private func downsizing() {
        var newArray = Array(repeating: defaultValue, count: size() * factor)
